...
class parser(object):
    def __init__(self):
        ...
        self.file_names = []
        self.file_types = [".mkv", ".mp4", ".mka", ".avi", ".mpeg", ".flv", ".mov", ".wmv"]
        self.post_title_pattern = '(?:{}|{})'.format(link_patterns(patterns['season']), link_patterns(patterns['year']))
            ...
            for file in files:
                for type in self.file_types:
                    if file.endswith(type):
                        # Some temp files getting generated with $ in the name
                        if "$" in file:
                            pass
                        else:
                            self.file_names.append([directory, file.lower()])

        self.file_names = sorted(self.file_names, key=lambda x : x[1])
